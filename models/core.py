...
class BLAN(FewShotClassifier):
    ...
    def forward_1shot(self, support_patch_prototypes, query_patch_features):
        ...
        query_patch_features = query_patch_features.unsqueeze(1).expand(-1, self.way, -1, -1)
        ...
        # distance_support_to_support = self.compute_distance(support_to_query_features, support_aggregated_prototypes)
        #
        # query_aggregated_features = query_aggregated_features.unsqueeze(1).expand(-1, self.way, -1)
        # distance_query_to_query = self.compute_distance(query_to_support_features, query_aggregated_features)
        #
        # if self.training:
        #     return distance_support_to_support, distance_query_to_query
        # else:
        #     return (distance_support_to_support + distance_query_to_query) / 2.0

        support_aggregated_prototypes = support_aggregated_prototypes.unsqueeze(0).expand(batch_size, -1, -1)
        sim_query_to_support = self.compute_distance(query_to_support_features, support_aggregated_prototypes)
        # return sim_query_to_support

        query_aggregated_features = query_aggregated_features.unsqueeze(1)
        sim_support_to_query = self.metric(query_aggregated_features, support_to_query_features).squeeze()
        if self.args.metric == 'cos':
            sim_support_to_query /= self.args.tau
        # return sim_support_to_query

        return self.weight * sim_support_to_query + (1 - self.weight) * sim_query_to_support

        # if self.training:
        #     return sim_support_to_query, sim_query_to_support
        # else:
        #     return (sim_support_to_query + sim_query_to_support) / 2.0
